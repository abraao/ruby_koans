...
class AboutRegularExpressions < Neo::Koan
  ...
  #
  # An alternation matches the whole thing or none of it. A character class matches a single character.
  #   Beyond that, it depends on the regex engine. Alternation can cause more backtracking since the
  #   engine consumes characters one by one, and if it doesn't end up matching an option in the
  #   alternation, it backtracks to the first character that matched in the word and tries again with
  #   another option in the alternation. It fails late instead of early, repeatedly if there's no match.
  ...
  def test_sub_is_like_find_and_replace
    # This:
    # 1. Matches "two" with t\w*
    # 2. Captures it in a capture group with the parenthesis.
    # 3. Accesses the capture group with $1
    # 4. Replaces the capture group "two" with its first character (i.e., [0, 1])
